...
const starts = async (dogexeon = new WAConnection()) => {
	...
	dogexeon.version = [2, 2143, 3]
	console.log(color(`\x1b[1;37m> ${tampilUcapan}\n`,'cyan'))
	console.log(color(figlet.textSync('NishiV3', {
		font: 'Standard',
		...
		if (anu.action == 'add') {
			...
			try {
				...
				ppUrl = await dogexeon.getProfilePicture(num)
					...
					ppUrl = 'https://i.pinimg.com/550x/f5/6b/ae/f56baef86aed6c261c422402aab59065.jpg'
			...
			try {
				...
				teks = `*Gᴏᴏᴅ Bʏᴇ* 👋\n@${num.split('@')[0]}\n*Demn Bruh, will miss that hoomun...* 👻`

				sendButImage(anu.jid, teks, `𝗡𝗶𝘀𝗵𝗶𝗩3`, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `GOODBYE !!`}, type: 1}], options = {contextInfo: {mentionedJid: [num, htod]}})
			...
			try {
				...
				sendButImage(anu.jid, teks, ``, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `Yo Boi, Promoted.. Happy?`}, type: 1}], options = {contextInfo: {mentionedJid: [num]}})
			...
			try {
				...
				sendButImage(anu.jid, teks, ``, img,but = [{buttonId: `Hello World!`, buttonText: {displayText: `Sad Life Bru`}, type: 1}], options = {contextInfo: {mentionedJid: [num]}})
